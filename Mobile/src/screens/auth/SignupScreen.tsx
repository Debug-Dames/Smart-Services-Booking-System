...
const FloatingInput: React.FC<{
  ...
  keyboardType?: any;
  autoCapitalize?: any;
  valid?: boolean | null;  // null = untouched, true = valid, false = invalid
  errorMsg?: string;
}> = ({
  label, value, onChangeText,
  secureTextEntry = false, keyboardType = 'default',
  autoCapitalize = 'none', valid = null, errorMsg,
}) => {
  ...
  const borderColor = borderAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [
      valid === false ? '#C03030' : theme.lavenderLight,
      valid === false ? '#C03030' : theme.primary,
    ],
  });
    ...
    <View style={{ marginBottom: 14 }}>
      <Animated.View style={[fi.wrap, { borderColor }]}>
        ...
        {valid === true && !secureTextEntry && (
          <View style={fi.checkWrap}>
            <Text style={{ fontSize: 13, color: '#1A8A50' }}>✓</Text>
          </View>
        )}
        {secureTextEntry && (
      ...
      {valid === false && errorMsg && (
        <Text style={fi.errorMsg}>{errorMsg}</Text>
      )}
    </View>
  );
};
      ...
      <View style={ps.bars}>
        {[1, 2, 3, 4].map(i => (
          <View
            ...
            style={[
              ps.bar,
              { backgroundColor: i <= strength ? colors[strength] : theme.lavenderLight },
            ]}
          />
        ))}
      </View>
...
const SignupScreen = ({ navigation }: any) => {
  ...
  const canSubmit =
    nameValid && emailValid && passwordValid && confirmValid && agreed;
  ...
  return (
    ...
    >
      ...
      {/* Top panel — smaller than login since more form fields */}
      <View style={su.topPanel}>
        <View style={su.panelCircle1} />
        <View style={su.panelCircle2} />
        <FadeIn delay={0}>
          <View style={su.monogramWrap}>
            <Text style={su.monogram}>D</Text>
          </View>
      ...
      <ScrollView
        contentContainerStyle={su.scroll}
      ...
      >
        ...
        <FadeIn delay={300}>
          ...
          >
            ...
            <Text style={su.termsText}>
              I agree to the{' '}
              <Text style={su.termsLink}>Terms of Service</Text>
              {' '}and{' '}
              <Text style={su.termsLink}>Privacy Policy</Text>
            </Text>
          </TouchableOpacity>
        </FadeIn>
        ...
        <FadeIn delay={380}>
          <TouchableOpacity
            style={[su.signupBtn, !canSubmit && su.signupBtnDisabled]}
            onPress={() => canSubmit && navigation.navigate('Home')}
            activeOpacity={canSubmit ? 0.85 : 1}
          >
            <Text style={su.signupBtnText}>Create Account</Text>
          </TouchableOpacity>
        </FadeIn>
...
const su = StyleSheet.create({
  ...
  panelCircle1: {
    ...
    backgroundColor: 'rgba(25,82,166,0.22)',
    top: -50, right: -30,
  },
  panelCircle2: {
    position: 'absolute',
  ...
  title: {
    ...
    color: theme.textPrimary, letterSpacing: -0.4, marginBottom: 4,
  },
  subtitle: {
  ...
  signupBtn: {
    ...
    shadowOpacity: 0.38, shadowRadius: 14,
