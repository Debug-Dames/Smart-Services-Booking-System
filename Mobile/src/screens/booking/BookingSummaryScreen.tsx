...
const INITIAL_BOOKINGS: Booking[] = [
  {
    id: '1', service: 'Balayage + Cut', emoji: '🎨',
    stylist: 'Ava Chen', date: 'Tue, Mar 3', time: '10:30 AM',
    duration: '2h 30m', price: 'R180', status: 'Upcoming',
  },
  {
    id: '2', service: 'Gel Manicure', emoji: '💅',
    stylist: 'Mia Torres', date: 'Thu, Mar 5', time: '2:00 PM',
    duration: '45 min', price: 'R45', status: 'Upcoming',
  },
  {
    id: '3', service: 'Blowout', emoji: '💨',
    stylist: 'James Liu', date: 'Sat, Feb 22', time: '11:00 AM',
    duration: '45 min', price: 'R45', status: 'Completed',
  },
  {
    id: '4', service: 'Haircut & Style', emoji: '✂️',
    stylist: 'Ava Chen', date: 'Mon, Feb 10', time: '3:00 PM',
    duration: '60 min', price: 'R65', status: 'Completed',
  },
  {
    id: '5', service: 'Deep Conditioning', emoji: '🌿',
    stylist: 'Priya Shah', date: 'Wed, Jan 29', time: '1:30 PM',
    duration: '30 min', price: 'R35', status: 'Cancelled',
  },
];
      ...
      <View style={bc.card}>
        ...
        <View style={bc.inner}>
          ...
          <View style={bc.topRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={bc.serviceName}>{item.service}</Text>
              <Text style={bc.stylistName}>with {item.stylist}</Text>
            </View>
            <View style={[bc.statusBadge, { backgroundColor: cfg.bg }]}>
              <View style={[bc.statusDot, { backgroundColor: cfg.dot }]} />
              <Text style={[bc.statusText, { color: cfg.text }]}>{item.status}</Text>
            </View>
          </View>
          ...
          <View style={bc.metaRow}>
            <View style={bc.metaItem}>
              ...
              <Text style={bc.metaValue}>{item.date}</Text>
            </View>
            <View style={bc.metaSep} />
            <View style={bc.metaItem}>
              <Text style={bc.metaLabel}>TIME</Text>
              <Text style={bc.metaValue}>{item.time}</Text>
            </View>
            <View style={bc.metaSep} />
            <View style={bc.metaItem}>
              <Text style={bc.metaLabel}>DURATION</Text>
              <Text style={bc.metaValue}>{item.duration}</Text>
            </View>
            <View style={bc.metaSep} />
            <View style={bc.metaItem}>
              <Text style={bc.metaLabel}>PRICE</Text>
              <Text style={[bc.metaValue, { color: theme.primary }]}>{item.price}</Text>
            </View>
          </View>
...
const bc = StyleSheet.create({
  ...
  stylistName: { fontSize: 12, color: theme.textSecondary, fontWeight: '500' },
  statusBadge: {
  ...
  metaLabel: {
    fontSize: 9, fontWeight: '700', color: theme.mutedText,
  ...
  cancelBtn: {
    ...
    borderWidth: 1.5, borderColor: '#E0BABA',
  ...
  rebookBtn: {
    flex: 1, paddingVertical: 10, borderRadius: 10,
    backgroundColor: 'rgba(25,82,166,0.1)', alignItems: 'center',
  },
...
// ─── Main Screen ──────────────────────────────────────────────────────────────
const FILTERS: BookingStatus[] = ['Upcoming', 'Completed', 'Cancelled'];
...
const MyBookingsScreen: React.FC<{ navigation?: any }> = ({ navigation }) => {
  ...
  const handleRebook = (item: Booking) => {
